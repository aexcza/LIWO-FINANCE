...
function deactivateUser(r,u){
  ...
  if(String(r.username)===String(u.username))throw Error("You cannot deactivate your own administrator account.");
  let sh=ss().getSheetByName("users"),v=sh.getDataRange().getValues(),i=v.slice(1).findIndex(x=>String(x[0]).toLowerCase()===String(r.username).toLowerCase());
